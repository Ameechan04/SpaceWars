package entity;

import javafx.scene.canvas.GraphicsContext;
import javafx.scene.shape.Ellipse;
import main.GamePanel;
import main.Star;
import javafx.scene.paint.Color;

public abstract class StationaryEntity extends Entity {
    public boolean hasVisibleOrbit = false;


    public StationaryEntity(GamePanel gamePanel, String name,Star currentStar, boolean hasVisibleOrbit, int buildCost, int maxHealth, int damage, Faction faction) {
        super(gamePanel, name, buildCost, maxHealth, damage, faction);
        this.currentStar = currentStar;
        orbitOffsetX = -12;
        orbitOffsetY = +10;
        this.hasVisibleOrbit = hasVisibleOrbit;

        defending = true; //stationary entities are always defending
        if (currentStar != null) {
            setCentrePosition(currentStar.x + orbitOffsetX, currentStar.y + orbitOffsetY);
        }

        // Load fallback image
        getImage();
    }





//    public void draw(Graphics2D g2) {
//
//        //new :
//        Image image = facingLeft ? left1 : right1;
//
//
//        if (debug) {
//            g2.setColor(Color.YELLOW);
//            g2.draw(new Rectangle(worldX, worldY, gamePanel.TILE_SIZE, gamePanel.TILE_SIZE));
//        }
//
//        gc.drawImage(image, imgX, imgY, solidArea.getWidth(), solidArea.getHeight());
//
//        g2.draw(image, worldX, worldY, gamePanel.TILE_SIZE, gamePanel.TILE_SIZE, null);
//
//    }

    public void drawOrbit(GraphicsContext gc) {
        if (this.faction == Faction.PLAYER) {
            gc.setStroke(Color.rgb(22, 64, 172));
        } else {
            gc.setStroke(Color.rgb(232, 23, 23));
        }
        double orbitWidth = 30;
        double orbitHeight = 30;
        gc.strokeOval(exactCentreX - orbitWidth / 2, exactCentreY - orbitHeight / 2, orbitWidth, orbitHeight);
    }

    public Ellipse createOrbit(int width, int height) {
        if (currentStar == null) return null;

        // Center the ellipse at the star's position, adjusting for width/height
        double x = currentStar.x - width / 2.0;
        double y = currentStar.y - height / 2.0;

        return new Ellipse(x, y, width, height);
    }

    protected void print_debug(){
        System.out.println("////////////");
        System.out.println("DEBUG");
        System.out.println("SOLID AREA:");
        System.out.println("x:" + solidArea.getX() + " y:" + solidArea.getY() + " width:" + solidArea.getWidth() + " height:" + solidArea.getHeight());
        System.out.println("Entity's central x,y: " + this.exactCentreX + ", " + this.exactCentreY);
        System.out.println("World x,y: " + this.worldX + ", " + this.worldY);
        System.out.println("Solid offset's x,y: " + this.solidOffsetX + ", " + this.solidOffsetY);


    }

}
